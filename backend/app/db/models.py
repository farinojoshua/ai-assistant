...
from sqlalchemy import Date, Float, ForeignKey, Integer, Numeric, String, Text, func
...
class WaLocation(Base):
    """A location shared via WhatsApp — one row per share, not a live tracker."""

    __tablename__ = "wa_locations"
...
class TicketBooking(Base):
    """A cinema ticket booking made via the SAMS Studios API (local audit copy)."""
    ...
    phone: Mapped[str] = mapped_column(String(20), index=True)
    partner_reference_number: Mapped[str] = mapped_column(
        String(64), unique=True, index=True
    )
    sams_customer_id: Mapped[str] = mapped_column(String(36))
    sams_booking_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    showtime_id: Mapped[str] = mapped_column(String(36))
    cinema_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
    movie_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
    showtime_start: Mapped[str | None] = mapped_column(String(40), nullable=True)
    seat_names: Mapped[str] = mapped_column(String(200))  # "G14, G15"
    amount: Mapped[float | None] = mapped_column(Numeric(14, 2), nullable=True)
    status: Mapped[str] = mapped_column(
        String(24), default="booked"
    )  # booked|paid|voided|failed
    payment_reference_number: Mapped[str | None] = mapped_column(
